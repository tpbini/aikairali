<?php
/**
 * Template part for rendering an AI Book list row item.
 *
...
// Get Category Name (book-category or fallback to post category)
...
$cat_name = ! empty( $categories ) && ! is_wp_error( $categories ) ? reset( $categories ) : __( 'AI BOOK', 'aikairali-portal' );

// Get Image URL
if ( has_post_thumbnail( $post_id ) ) {
	$image_url = get_the_post_thumbnail_url( $post_id, 'medium' );
} else {
	$cover = get_field( 'book_cover', $post_id );
	if ( is_array( $cover ) && isset( $cover['url'] ) ) {
		$image_url = $cover['url'];
	} elseif ( is_string( $cover ) && ! empty( $cover ) ) {
		$image_url = $cover;
	} else {
