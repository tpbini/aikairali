...
$free_plan        = get_field( 'free_plan' );
$api_available    = get_field( 'api_available' );
$ai_model         = get_field( 'ai_model_used' );
...
<div class="aikairali-detail-container" style="max-width: 1200px; margin: 0 auto; padding: 40px 20px; font-family: system-ui, -apple-system, sans-serif;">
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<!-- 2. Main 2-Column Grid (~70% left, ~30% right) -->
		<div style="display: grid; grid-template-columns: 2.2fr 1fr; gap: 48px;">
			...
			<main>
				...
				<!-- Structured Specification Box (In Left Column!) -->
				<div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; margin: 32px 0;">
					...
					<h2 style="font-size: 1.75rem; font-weight: 800; color: #0f172a; margin: 0 0 20px 0;">
						<?php echo esc_html( $pricing ?: 'Free' ); ?>
					</h2>
					...
					<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px 24px; font-size: 0.925rem;">
						<div style="display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
							<span style="color: #64748b;">Free Plan:</span>
							<strong style="color: <?php echo $free_plan ? '#16a34a' : '#dc2626'; ?>;">
								<?php echo $free_plan ? 'Yes' : 'No'; ?>
							</strong>
						</div>
						<div style="display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
							<span style="color: #64748b;">API Available:</span>
							<strong style="color: <?php echo $api_available ? '#16a34a' : '#94a3b8'; ?>;">
								<?php echo $api_available ? '✓ Yes' : '—'; ?>
							</strong>
						</div>
						<?php if ( $developer ) : ?>
						<div style="display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
							<span style="color: #64748b;">Developer:</span>
							<strong><?php echo esc_html( $developer ); ?></strong>
						</div>
						<?php endif; ?>
						<?php if ( $ai_model ) : ?>
						<div style="display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
							<span style="color: #64748b;">AI Model:</span>
							<strong><?php echo esc_html( $ai_model ); ?></strong>
						</div>
						<?php endif; ?>
						<?php if ( $rating ) : ?>
						<div style="display: flex; justify-content: space-between; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">
							<span style="color: #64748b;">Rating:</span>
							<strong style="color: #d97706;">★ <?php echo esc_html( $rating ); ?> / 5</strong>
						</div>
						<?php endif; ?>
					</div>
					...
					<?php if ( $official_website ) : ?>
						<div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #e2e8f0;">
							<a href="<?php echo esc_url( $official_website ); ?>" target="_blank" rel="noopener" style="display: inline-block; background: #0284c7; color: #ffffff; font-weight: 700; padding: 10px 22px; border-radius: 8px; text-decoration: none;">
