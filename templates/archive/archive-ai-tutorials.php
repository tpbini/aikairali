<?php
/**
 * Archive / List Template for AI Tutorials.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_tutorials_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_tutorials_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Tutorials Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_tutorials_view() {
	$categories_config = [
		[
			'name' => 'Beginner Guides',
			'slug' => 'beginner-guides',
			'desc' => 'Introductory guides to artificial intelligence, prompting basics, and core concepts',
		],
		[
			'name' => 'Coding & Dev',
			'slug' => 'coding-dev',
			'desc' => 'Python AI scripts, PyTorch, OpenAI API, LangChain, and API integrations',
		],
		[
			'name' => 'Prompt Engineering',
			'slug' => 'prompt-engineering',
			'desc' => 'Advanced techniques, system instructions, and chain-of-thought prompting',
		],
		[
			'name' => 'Video & Audio AI',
			'slug' => 'video-audio-ai',
			'desc' => 'Sora, ElevenLabs, Suno AI, Runway Gen-2 tutorials and generative media',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Tutorials & Guides', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Step-by-step guides, code tutorials, and artificial intelligence learning resources in Malayalam.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-tutorials-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_tutorials_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_tutorials_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'tutorial-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-tutorials' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-tutorials', 'tutorial' ],
		'tax_query'      => [
			[
	...
	if ( $cat_posts_query->have_posts() ) {
		while ( $cat_posts_query->have_posts() ) {
			...
			$posts[] = [
				...
				'image' => $img ?: 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600&auto=format&fit=crop',
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_tutorials_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_tutorials_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Tutorials' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Tutorials: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence guides and code tutorials in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_tutorials_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_tutorials_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_tutorials_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT TUTORIALS</h3>
				</div>
...
/**
 * Return customized fallback tutorial items per category
 */
function aikairali_get_fallback_tutorials_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s പൂർണ്ണ ട്യൂട്ടോറിയലും മലയാളം പഠന സഹായിയും', $cat_name ),
			'excerpt' => 'ഘട്ടം ഘട്ടമായി വികസിപ്പിക്കുന്ന പ്രാക്ടിക്കൽ കോഡിംഗും കൺസെപ്റ്റ് വിവരണങ്ങളും...',
			'image' => 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പൈത്തൺ & ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് പ്രോജക്ട് ഗൈഡ്', $cat_name ),
			'excerpt' => 'സ്വന്തമായി എഐ ആപ്ലിക്കേഷനുകൾ നിർമ്മിക്കാൻ സഹായിക്കുന്ന ആധുനിക കോഡിംഗ് ട്യൂട്ടോറിയൽ...',
			'image' => 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഓപ്പൺഎഐ എപിഐ ഇൻടഗ്രേഷൻ ട്യൂട്ടോറിയൽ 2026', $cat_name ),
			'excerpt' => 'നിങ്ങളുടെ വെബ്‌സൈറ്റിലും ആപ്പുകളിലും ചാറ്റ്ജിപിടി സാങ്കേതികവിദ്യ സംയോജിപ്പിക്കുക...',
			'image' => 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s അഡ്വാൻസ്ഡ് പ്രോംപ്റ്റിംഗ് സീക്വൻസ് & മാസ്റ്ററി', $cat_name ),
			'excerpt' => 'സങ്കീർണ്ണമായ ടാസ്കുകൾ പരിഹരിക്കാൻ സഹായിക്കുന്ന പ്രോംപ്റ്റ് ചെയിനിംഗ് മാർഗ്ഗങ്ങൾ...',
			'image' => 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് ജനറേറ്റീവ് മീഡിയ ഗൈഡ്', $cat_name ),
			'excerpt' => 'ഓഡിയോ, വീഡിയോ, ഇമേജ് ജനറേഷൻ ടൂളുകൾ പ്രൊഫഷണലായി ഉപയോഗിക്കാൻ പഠിക്കാം...',
...
/**
 * Helper function to render a single DB tutorial item in stream list view
 */
function aikairali_render_tutorials_list_item( $post_id ) {
	...
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI TUTORIAL';

	aikairali_render_tutorials_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a tutorial item array structure matching stream design
 */
function aikairali_render_tutorials_default_item( $item ) {
	?>
