<?php
/**
 * Archive / List Template for AI Videos.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_videos_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_videos_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Videos Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_videos_view() {
	$categories_config = [
		[
			'name' => 'Video Tutorials',
			'slug' => 'video-tutorials',
			'desc' => 'Step-by-step video lessons and software walk-throughs',
		],
		[
			'name' => 'Product Demos',
			'slug' => 'product-demos',
			'desc' => 'AI tool reviews, live demos, and comparative video tests',
		],
		[
			'name' => 'Keynotes & Talks',
			'slug' => 'keynotes-talks',
			'desc' => 'AI developer conferences, tech talks, and expert interviews',
		],
		[
			'name' => 'Malayalam AI Class',
			'slug' => 'malayalam-ai-class',
			'desc' => 'Malayalam video masterclasses and technology explanations',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Videos & Demos', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Watch video tutorials, AI software demonstrations, and tech webinars in Malayalam.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-videos-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_videos_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_videos_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'video-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-videos' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-videos', 'videos' ],
		'tax_query'      => [
			[
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_videos_for_category( $cat_slug, $cat_name );
	}
		...
		<div class="ai-cat-grid-layout" style="display: flex; gap: 24px; flex-wrap: wrap;">
			...
			<div class="ai-cat-feature-col" style="flex: 1.1; min-width: 280px;">
				<article class="ai-cat-feature-card" style="background: #ffffff; border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; height: 100%;">
					<a href="<?php echo esc_url( $feature_post['link'] ); ?>" style="display: block; width: 100%; aspect-ratio: 16/10; overflow: hidden; border-radius: 12px; background: #f1f5f9; margin-bottom: 14px; position: relative;">
						...
						<div style="position: absolute; inset: 0; background: rgba(0,0,0,0.25); display: flex; align-items: center; justify-content: center;">
							<div style="width: 44px; height: 44px; border-radius: 50%; background: rgba(37,99,235,0.9); display: flex; align-items: center; justify-content: center; color: #ffffff;">
				...
				<div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
					<?php foreach ( $grid_posts as $g_item ) : ?>
						<article class="ai-cat-small-card" style="display: flex; flex-direction: column;">
							<a href="<?php echo esc_url( $g_item['link'] ); ?>" style="display: block; width: 100%; aspect-ratio: 16/10; overflow: hidden; border-radius: 10px; background: #f1f5f9; margin-bottom: 10px; position: relative;">
								...
								<div style="position: absolute; inset: 0; background: rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center;">
									<div style="width: 32px; height: 32px; border-radius: 50%; background: rgba(37,99,235,0.9); display: flex; align-items: center; justify-content: center; color: #ffffff;">
 ...
 */
function aikairali_render_single_category_videos_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Videos' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Videos: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Watch all artificial intelligence video tutorials and demos in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_videos_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_videos_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_videos_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT VIDEOS</h3>
				</div>
...
/**
 * Return customized fallback video items per category
 */
function aikairali_get_fallback_videos_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s പൂർണ്ണ വീഡിയോ പാഠവും മലയാളം വിശദീകരണവും', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി സാങ്കേതികവിദ്യ പ്രായോഗികമായി ഉപയോഗിക്കുന്ന വിധം വ്യക്തമാക്കുന്ന പ്രാക്ടിക്കൽ വീഡിയോ...',
			'image' => 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പുതിയ അപ്‌ഡേറ്റ് ലൈവ് റിവ്യൂ & ഡെമോ', $cat_name ),
			'excerpt' => 'അത്യാധുനിക ഫീച്ചറുകളും പ്രകടനക്ഷമതയും വിലയിരുത്തുന്ന തത്സമയ ഫീച്ചർ ഡെമോൺസ്ട്രേഷൻ...',
			'image' => 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പ്രൊഫഷണൽ വെബിനാർ & ക്വസ്റ്റ്യൻ സെഷൻ', $cat_name ),
			'excerpt' => 'രംഗത്തെ വിദഗ്ദ്ധരുമായി നടത്തിയ സംവാദവും സാങ്കേതിക ചോദ്യോത്തര വേളകളും...',
			'image' => 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഹാൻഡ്സ് ഓൺ സാങ്കേതിക ഗൈഡ് വീഡിയോ', $cat_name ),
			'excerpt' => 'സ്റ്റെപ്പ് ബൈ സ്റ്റെപ്പ് ആപ്ലിക്കേഷൻ ഡെവലപ്പ്മെന്റ് വീഡിയോ ട്യൂട്ടോറിയൽ...',
			'image' => 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s എഐ ബിസിനസ്സ് ആപ്ലിക്കേഷനുകൾ ഡെമോ', $cat_name ),
			'excerpt' => 'വ്യാവസായിക സ്ഥാപനങ്ങളിൽ എഐ നടപ്പിലാക്കുന്ന വിധം വ്യക്തമാക്കുന്ന വീഡിയോ...',
...
/**
 * Helper function to render a single DB video item in stream list view
 */
function aikairali_render_videos_list_item( $post_id ) {
	...
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI VIDEO';

	aikairali_render_videos_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a video item array structure matching stream design
 */
function aikairali_render_videos_default_item( $item ) {
	?>
	<article class="ai-news-card-item" style="display: flex; gap: 24px; padding-bottom: 24px; margin-bottom: 24px; border-bottom: 1px solid #e2e8f0; align-items: flex-start;">
		<a href="<?php echo esc_url( $item['link'] ); ?>" class="ai-news-thumb-link" style="display: block; width: 240px; min-width: 240px; aspect-ratio: 16/10; border-radius: 14px; overflow: hidden; background: #f1f5f9; flex-shrink: 0; box-shadow: 0 2px 8px rgba(0,0,0,0.04); position: relative;">
			...
			<div style="position: absolute; inset: 0; background: rgba(0,0,0,0.25); display: flex; align-items: center; justify-content: center;">
				<div style="width: 40px; height: 40px; border-radius: 50%; background: rgba(37,99,235,0.9); display: flex; align-items: center; justify-content: center; color: #ffffff;">
