<?php
/**
 * Archive / List Template for AI Events.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_events_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_events_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Events Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_events_view() {
	$categories_config = [
		[
			'name' => 'Conferences & Summits',
			'slug' => 'conferences-summits',
			'desc' => 'Major AI technology summits, global expos, and industry conventions',
		],
		[
			'name' => 'Hackathons',
			'slug' => 'hackathons',
			'desc' => 'AI hackathons, coding competitions, and startup pitch challenges',
		],
		[
			'name' => 'Webinars & Workshops',
			'slug' => 'webinars-workshops',
			'desc' => 'Online masterclasses, live coding sessions, and hands-on workshops',
		],
		[
			'name' => 'Kerala AI Meetups',
			'slug' => 'kerala-ai-meetups',
			'desc' => 'Local tech meetups in Kochi, Trivandrum, Kozhikode, and Thrissur',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Events & Conferences', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Upcoming artificial intelligence summits, hackathons, webinars, and tech conferences.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-events-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_events_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_events_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'event-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-events' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-events', 'events' ],
		'tax_query'      => [
			[
	...
	if ( $cat_posts_query->have_posts() ) {
		while ( $cat_posts_query->have_posts() ) {
			...
			$posts[] = [
				...
				'image' => $img ?: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=600&auto=format&fit=crop',
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_events_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_events_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Events' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Events: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence events and summits in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_events_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_events_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_events_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT EVENTS</h3>
				</div>
...
/**
 * Return customized fallback event items per category
 */
function aikairali_get_fallback_events_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s ആഗോള എഐ ഉച്ചകോടിയും സാങ്കേതിക കോൺഫറൻസും 2026', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി വികസനത്തെക്കുറിച്ചും ഭാവി സാധ്യതകളെക്കുറിച്ചുമുള്ള അന്താരാഷ്ട്ര സമ്മേളനം...',
			'image' => 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഹാൻഡ്സ് ഓൺ ആപ്പുകൾ നിർമ്മിക്കാനുള്ള എഐ ഹാക്കത്തോൺ', $cat_name ),
			'excerpt' => 'ഡെവലപ്പർമാർക്കും വിദ്യാർത്ഥികൾക്കും പങ്കെടുക്കാവുന്ന 48 മണിക്കൂർ സാങ്കേതിക മത്സരം...',
			'image' => 'https://images.unsplash.com/photo-1515187029135-18ee286d815b?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s തത്സമയ വെബിനാറും വിദഗ്ദ്ധ പരിശീലന ക്ലാസ്സും', $cat_name ),
			'excerpt' => 'ആഗോള എഐ സ്ട്രാറ്റജികൾ അടുത്തറിയാനുള്ള സൗജന്യ വെബിനാർ പ്രോഗ്രാം...',
			'image' => 'https://images.unsplash.com/photo-1505373877841-8d25f7d46678?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s കേരള എഐ സ്റ്റാർട്ടപ്പ് മീറ്റപ്പ് & ഇൻവെസ്റ്റർ കണക്ട്', $cat_name ),
			'excerpt' => 'കേരളത്തിലെ എഐ സംരംഭകർക്കായി നടത്തുന്ന പ്രൊഫഷണൽ നെറ്റ്വർക്കിംഗ് സംഗമം...',
			'image' => 'https://images.unsplash.com/photo-1511578314322-379afb476865?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പുതിയ തലമുറ എഐ ടെക്നോളജി എക്സിബിഷൻ', $cat_name ),
			'excerpt' => 'അത്യാധുനിക റോബോട്ടിക്സ്, വോയിസ് എഐ സാങ്കേതികവിദ്യകൾ പ്രദർശിപ്പിക്കുന്ന മേള...',
...
/**
 * Helper function to render a single DB event item in stream list view
 */
function aikairali_render_events_list_item( $post_id ) {
	...
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI EVENT';

	aikairali_render_events_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render an event item array structure matching stream design
 */
function aikairali_render_events_default_item( $item ) {
	?>
