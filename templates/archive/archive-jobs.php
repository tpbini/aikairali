<?php
/**
 * Archive / List Template for Jobs.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_jobs_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_jobs_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main Jobs Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_jobs_view() {
	$categories_config = [
		[
			'name' => 'AI Engineering',
			'slug' => 'ai-engineering',
			'desc' => 'Machine learning engineers, AI software developers, and PyTorch developers',
		],
		[
			'name' => 'Prompt Engineering',
			'slug' => 'prompt-engineering',
			'desc' => 'Prompt engineers, AI content specialists, and system designers',
		],
		[
			'name' => 'Data Science',
			'slug' => 'data-science',
			'desc' => 'Data scientists, MLOps engineers, and data analytics specialists',
		],
		[
			'name' => 'Remote Jobs',
			'slug' => 'remote-jobs',
			'desc' => 'Remote global artificial intelligence opportunities and freelance contracts',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Jobs & Careers Directory', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Discover artificial intelligence, machine learning, prompt engineering, and tech career opportunities in Kerala and remotely.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-jobs-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_jobs_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_jobs_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'job-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'jobs' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'jobs', 'ai-jobs' ],
		'tax_query'      => [
			[
	...
	if ( $cat_posts_query->have_posts() ) {
		while ( $cat_posts_query->have_posts() ) {
			...
			$posts[] = [
				...
				'image' => $img ?: 'https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=600&auto=format&fit=crop',
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_jobs_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_jobs_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Jobs' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Jobs: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence career opportunities in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_jobs_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_jobs_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_jobs_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT JOBS</h3>
				</div>
...
/**
 * Return customized fallback job items per category
 */
function aikairali_get_fallback_jobs_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s സീനിയർ എഐ എൻജിനീയർ അടിയന്തിര ഒഴിവുകൾ 2026', $cat_name ),
			'excerpt' => 'കൊച്ചി ഇൻഫോപാർക്കിലും ടെക്നോപാർക്കിലും അത്യാകർഷകമായ ശമ്പളത്തിൽ പുതിയ തൊഴിൽ അവസരങ്ങൾ...',
			'image' => 'https://images.unsplash.com/photo-1486312338219-ce68d2c6f44d?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പ്രൊഫഷണൽ പ്രോംപ്റ്റ് ആർക്കിടെക്റ്റ് റിമോട്ട് ജോലികൾ', $cat_name ),
			'excerpt' => 'ആഗോള തലത്തിൽ വീട്ടിലിരുന്ന് ജോലി ചെയ്യാവുന്ന എഐ ഡെവലപ്പർ ആപ്പറ്റൈറ്റ് വേക്കൻസികൾ...',
			'image' => 'https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ലീഡ് ഡാറ്റാ സയന്റിസ്റ്റ് & MLOps എൻജിനീയർ', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി മോഡലുകൾ വികസിപ്പിക്കുന്ന സ്ഥാപനങ്ങളിലേക്ക് തരംഗമാകുന്ന പുതിയ അവസരങ്ങൾ...',
			'image' => 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ജൂനിയർ എഐ സ്പെഷ്യലിസ്റ്റ് & ഇന്റേൺഷിപ്പ് പ്രോഗ്രാം', $cat_name ),
			'excerpt' => 'ഫ്രഷേഴ്സിനും പഠിച്ചിറങ്ങുന്ന വിദ്യാർത്ഥികൾക്കും പ്രായോഗിക തൊഴിൽ പരിശീലനം നൽകുന്ന കോഴ്സ് സീറ്റുകൾ...',
			'image' => 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s എഐ ബിസിനസ്സ് കൺസൾട്ടന്റ് & പ്രോജക്ട് മാനേജർ', $cat_name ),
			'excerpt' => 'മൾട്ടിനാഷണൽ കമ്പനികളിലേക്ക് സ്ട്രാറ്റജിക് മാനേജർമാരെ ആവശ്യമുണ്ട്...',
...
/**
 * Helper function to render a single DB job item in stream list view
 */
function aikairali_render_jobs_list_item( $post_id ) {
	...
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI JOB';

	aikairali_render_jobs_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a job item array structure matching stream design
 */
function aikairali_render_jobs_default_item( $item ) {
	?>
