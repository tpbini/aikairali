<?php
/**
 * Archive / List Template for AI Tools.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_tools_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_tools_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Tools Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_tools_view() {
	$categories_config = [
		[
			'name' => 'Text & Writing',
			'slug' => 'text-writing',
			'desc' => 'ChatGPT, Claude 3.5, Gemini 1.5, Jasper, and AI writing assistants',
		],
		[
			'name' => 'Image & Art',
			'slug' => 'image-art',
			'desc' => 'Midjourney v6, Stable Diffusion, DALL-E 3, and digital artwork generators',
		],
		[
			'name' => 'Video & Animation',
			'slug' => 'video-animation',
			'desc' => 'Runway Gen-3, Pika Labs, HeyGen, and AI video generators',
		],
		[
			'name' => 'Code & Developer',
			'slug' => 'code-developer',
			'desc' => 'GitHub Copilot, Cursor AI, Replit Agent, and coding assistants',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Tools Directory', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Discover and compare the best artificial intelligence software, apps, and web services.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-tools-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_tools_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_tools_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'tool-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-tools' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-tools', 'tools' ],
		'tax_query'      => [
			[
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_tools_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_tools_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Tools' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Tools: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence software and tools in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_tools_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_tools_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_tools_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT TOOLS</h3>
				</div>
...
/**
 * Return customized fallback tool items per category
 */
function aikairali_get_fallback_tools_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s അത്യാധുനിക ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് ടൂളുകൾ 2026', $cat_name ),
			'excerpt' => 'നിങ്ങളുടെ ദൈനംദിന ജോലികൾ എളുപ്പമാക്കുന്ന പുതിയ എഐ ആപ്ലിക്കേഷനുകളുടെ സമഗ്ര വിശകലനം...',
			'image' => 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s മികച്ച സൗജന്യ & പ്രീമിയം ടൂളുകളുടെ താരതമ്യം', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി വിപണിയിൽ തരംഗമാകുന്ന ജനപ്രിയ ടൂളുകളുടെ ഫീച്ചറുകളും പ്ലാനുകളും...',
			'image' => 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഉൽപ്പാദനക്ഷമത 10x വർദ്ധിപ്പിക്കുന്ന സോഫ്റ്റ്‌വെയറുകൾ', $cat_name ),
			'excerpt' => 'സ്ഥാപനങ്ങൾക്കും സ്വതന്ത്ര സംരംഭകർക്കും അനുയോജ്യമായ എഐ ആപ്പുകൾ...',
			'image' => 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ക്രിയേറ്റീവ് വർക്കുകൾക്ക് അനുയോജ്യമായ എഐ സേവനങ്ങൾ', $cat_name ),
			'excerpt' => 'ഡിസൈനർമാർക്കും കണ്ടന്റ് ക്രിയേറ്റർമാർക്കും ഉപയോഗപ്രദമായ ഡിജിറ്റൽ ടൂളുകൾ...',
			'image' => 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s സെക്യൂർ ഓട്ടോമേഷൻ ബിസിനസ്സ് ടൂൾസ്', $cat_name ),
			'excerpt' => 'സുരക്ഷിതമായി ഡാറ്റ ഹാൻഡിൽ ചെയ്യുന്ന പ്രൊഫഷണൽ എഐ സാങ്കേതികവിദ്യകൾ...',
...
/**
 * Helper function to render a single DB tool item in stream list view
 */
function aikairali_render_tools_list_item( $post_id ) {
	...
	$terms    = get_the_terms( $post_id, 'tool-category' );
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI TOOL';

	aikairali_render_tools_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a tool item array structure matching stream design
 */
function aikairali_render_tools_default_item( $item ) {
	?>
