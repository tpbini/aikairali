<?php
/**
 * Archive / List Template for AI Books.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_books_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_books_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Books Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_books_view() {
	$categories_config = [
		[
			'name' => 'ML Fundamentals',
			'slug' => 'ml-fundamentals',
			'desc' => 'Core machine learning algorithms, mathematics, and Python programming textbooks',
		],
		[
			'name' => 'Generative AI & LLMs',
			'slug' => 'generative-ai-llms',
			'desc' => 'Transformer architecture, large language model engineering, and deep learning',
		],
		[
			'name' => 'Prompt Engineering',
			'slug' => 'prompt-engineering',
			'desc' => 'Practical guides, strategy handbooks, and prompt engineering best practices',
		],
		[
			'name' => 'AI Ethics & Society',
			'slug' => 'ai-ethics-society',
			'desc' => 'Future of artificial general intelligence, AI safety, ethics, and economic impact',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Books Directory', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Explore essential artificial intelligence, machine learning, and prompt engineering books in Malayalam and English.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-books-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_books_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_books_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'book-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-books' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-books', 'books' ],
		'tax_query'      => [
			[
	...
	if ( $cat_posts_query->have_posts() ) {
		while ( $cat_posts_query->have_posts() ) {
			...
			$posts[] = [
				...
				'image' => $img ?: 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=600&auto=format&fit=crop',
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_books_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_books_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Books' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Books: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence books in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_books_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_books_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_books_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT BOOKS</h3>
				</div>
...
/**
 * Return customized fallback book items per category
 */
function aikairali_get_fallback_books_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s പ്രധാന പുസ്തകങ്ങളും ഗ്രന്ഥ വിശകലനവും', $cat_name ),
			'excerpt' => 'ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് രംഗത്തെ പ്രമുഖർ രചിച്ച അത്യാധുനിക ടെക്സ്റ്റ് ബുക്കുകളും ഗൈഡുകളും...',
			'image' => 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പ്രാക്ടിക്കൽ ഗൈഡ് മാസ്റ്ററി ഫോർ ബിഗിനേഴ്സ്', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി തത്വങ്ങൾ എളുപ്പത്തിൽ മനസിലാക്കാൻ സഹായിക്കുന്ന വിവരണം...',
			'image' => 'https://images.unsplash.com/photo-1512820790803-83ca734da794?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s അഡ്വാൻസ്ഡ് ടെക്നോളജി ബുക്ക് കളക്ഷൻ 2026', $cat_name ),
			'excerpt' => 'മെഷീൻ ലേണിംഗ്, പ്രോംപ്റ്റ് ഡിസൈൻ, ന്യൂറൽ നെറ്റ്‌വർക്കുകൾ എന്നിവ വിശദമാക്കുന്ന ഗ്രന്ഥം...',
			'image' => 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഫ്യൂച്ചർ ഓഫ് ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് ബുക്ക് ഗൈഡ്', $cat_name ),
			'excerpt' => 'ഭാവി ലോകത്ത് എഐ സൃഷ്ടിക്കുന്ന മാറ്റങ്ങളെക്കുറിച്ചുള്ള സമഗ്ര പഠന റിപ്പോർട്ട്...',
			'image' => 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ബിസിനസ്സ് ലീഡർഷിപ്പ് എഐ സ്ട്രാറ്റജി ബുക്ക്', $cat_name ),
			'excerpt' => 'സ്ഥാപനങ്ങളിൽ എഐ നിർമ്മിക്കാൻ ആഗ്രഹിക്കുന്നവർക്കുള്ള സമഗ്ര വഴികാട്ടി...',
...
/**
 * Helper function to render a single DB book item in stream list view
 */
function aikairali_render_books_list_item( $post_id ) {
	...
	$terms    = get_the_terms( $post_id, 'book-category' );
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI BOOK';

	aikairali_render_books_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a book item array structure matching stream design
 */
function aikairali_render_books_default_item( $item ) {
	?>
