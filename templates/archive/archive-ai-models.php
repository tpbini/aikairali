<?php
/**
 * Archive / List Template for AI Models.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_models_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_models_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Models Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_models_view() {
	$categories_config = [
		[
			'name' => 'Large Language Models',
			'slug' => 'large-language-models',
			'desc' => 'GPT-4o, Claude 3.5 Sonnet, Gemini 1.5 Pro, and LLM benchmarks',
		],
		[
			'name' => 'Vision & Multimodal',
			'slug' => 'vision-multimodal',
			'desc' => 'GPT-4V, Gemini Vision, CLIP, Florence-2, and image-to-text models',
		],
		[
			'name' => 'Open Source Weights',
			'slug' => 'open-source-weights',
			'desc' => 'Llama 3.1, Mistral Large 2, Qwen 2.5, and open weights for self-hosting',
		],
		[
			'name' => 'Speech & Audio Models',
			'slug' => 'speech-audio-models',
			'desc' => 'Whisper, SeamlessM4T, ElevenLabs, Suno, and speech recognition',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Models & LLMs Directory', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Explore large language models, open-source AI weights, vision models, and neural architecture benchmarks.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-models-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_models_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_models_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'model-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-models' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-models', 'models' ],
		'tax_query'      => [
			[
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_models_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_models_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Models' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Models: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence neural models in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_models_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_models_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_models_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT MODELS</h3>
				</div>
...
/**
 * Return customized fallback model items per category
 */
function aikairali_get_fallback_models_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s പുതിയ അപ്‌ഡേറ്റുകളും ബെഞ്ച്മാർക്ക് സ്കോറുകളും', $cat_name ),
			'excerpt' => 'അത്യാധുനിക എഐ മോഡലുകളുടെ പ്രകടനക്ഷമതയും സാങ്കേതിക സവിശേഷതകളും...',
			'image' => 'https://images.unsplash.com/photo-1677442136019-21780efad99a?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s തദ്ദേശീയ സപ്പോർട്ടുള്ള അത്യാധുനിക മോഡലുകൾ', $cat_name ),
			'excerpt' => 'പ്രാദേശിക ഭാഷകളിലും പ്രോസസ്സിംഗിലും ഉയർന്ന അക്യുറസി നൽകുന്ന എഐ സിസ്റ്റം...',
			'image' => 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ഓപ്പൺ സോഴ്സ് സിസ്റ്റങ്ങളും ലോക്കൽ സെൽഫ് ഹോസ്റ്റിംഗും', $cat_name ),
			'excerpt' => 'സ്വന്തം സെർവറുകളിൽ സുരക്ഷിതമായി എഐ മോഡലുകൾ പ്രവർത്തിപ്പിക്കാനുള്ള ഗൈഡ്...',
			'image' => 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s മൾട്ടിമോഡൽ വോയിസ് & വിഷ്വൽ എഐ ട്രെൻഡുകൾ', $cat_name ),
			'excerpt' => 'ചിത്രങ്ങളും ശബ്ദവും ഒരേ സമയം പ്രോസസ്സ് ചെയ്യുന്ന പുതിയ സാങ്കേതികവിദ്യ...',
			'image' => 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s എന്റർപ്രൈസ് എഐ മോഡൽ ഇൻടഗ്രേഷൻ സ്ട്രാറ്റജി', $cat_name ),
			'excerpt' => 'വലിയ വ്യവസായ സ്ഥാപനങ്ങളിൽ എഐ മോഡലുകൾ ഫൈൻ ട്യൂൺ ചെയ്യുന്ന വിധം...',
...
/**
 * Helper function to render a single DB model item in stream list view
 */
function aikairali_render_models_list_item( $post_id ) {
	...
	$terms    = get_the_terms( $post_id, 'model-category' );
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'AI MODEL';

	aikairali_render_models_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a model item array structure matching stream design
 */
function aikairali_render_models_default_item( $item ) {
	?>
