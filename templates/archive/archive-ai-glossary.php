<?php
/**
 * Archive / List Template for AI Glossary.
...
<div class="aikairali-container" style="max-width: 1200px; margin: 40px auto; padding: 0 20px; box-sizing: border-box; font-family: 'Anek Malayalam', system-ui, -apple-system, sans-serif;">
	...
	<?php if ( $is_single_category ) : ?>
		<?php aikairali_render_single_category_glossary_view( $current_cat ); ?>
	<?php else : ?>
		<?php aikairali_render_multi_category_glossary_view(); ?>
	<?php endif; ?>
...
<?php
/**
 * Render Main AI Glossary Multi-Category Sectioned Layout
 */
function aikairali_render_multi_category_glossary_view() {
	$categories_config = [
		[
			'name' => 'Core AI Terms',
			'slug' => 'core-ai-terms',
			'desc' => 'Machine learning, neural networks, deep learning, NLP, and algorithms',
		],
		[
			'name' => 'Generative AI Terms',
			'slug' => 'generative-ai-terms',
			'desc' => 'Transformers, attention mechanism, embeddings, vector databases, and RAG',
		],
		[
			'name' => 'Prompting Terms',
			'slug' => 'prompting-terms',
			'desc' => 'Zero-shot, few-shot, chain-of-thought, prompt injection, and hallucinations',
		],
		[
			'name' => 'Ethics & Safety',
			'slug' => 'ethics-safety',
			'desc' => 'AI alignment, algorithmic bias, guardrails, RLHF, and AI safety concepts',
		],
	];
	?>
	...
	<header class="archive-header" style="margin-bottom: 40px; border-bottom: 2px solid #f1f5f9; padding-bottom: 24px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php esc_html_e( 'AI Glossary & Terminology', 'aikairali-portal' ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php esc_html_e( 'Clear definitions and explanations for artificial intelligence, machine learning, and tech terms in Malayalam.', 'aikairali-portal' ); ?>
		</p>
	</header>

	<!-- Iterate over each Category Section -->
	<div class="ai-glossary-categories-wrapper">
		<?php foreach ( $categories_config as $cat_info ) : ?>
			<?php aikairali_render_glossary_category_section_block( $cat_info ); ?>
		<?php endforeach; ?>
	</div>
	<?php
}
 ...
 */
function aikairali_render_glossary_category_section_block( $cat_info ) {
	$cat_name = $cat_info['name'];
	$cat_slug = $cat_info['slug'];
	
	$term     = get_term_by( 'slug', $cat_slug, 'glossary-category' );
	$cat_link = $term ? get_term_link( $term ) : add_query_arg( 'cat', $cat_slug, get_post_type_archive_link( 'ai-glossary' ) );

	$cat_posts_query = new WP_Query( [
		'post_type'      => [ 'ai-glossary', 'glossary' ],
		'tax_query'      => [
			[
	...
	if ( $cat_posts_query->have_posts() ) {
		while ( $cat_posts_query->have_posts() ) {
			...
			$posts[] = [
				...
				'image' => $img ?: 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=600&auto=format&fit=crop',
	...
	if ( empty( $posts ) ) {
		$posts = aikairali_get_fallback_glossary_for_category( $cat_slug, $cat_name );
	}
 ...
 */
function aikairali_render_single_category_glossary_view( $term ) {
	$cat_name = is_object( $term ) && isset( $term->name ) ? $term->name : ( isset( $_GET['cat'] ) ? sanitize_text_field( $_GET['cat'] ) : 'AI Glossary' );
	...
	<header class="archive-header" style="margin-bottom: 35px; border-bottom: 2px solid #f1f5f9; padding-bottom: 20px;">
		<div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
			...
			<h1 style="font-size: 26px; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.2;">
				<?php printf( esc_html__( 'AI Glossary: %s', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
			</h1>
		</div>
		<p style="color: #64748b; font-size: 16px; margin: 0;">
			<?php printf( esc_html__( 'Browse all artificial intelligence terminology and definitions in %s.', 'aikairali-portal' ), esc_html( $cat_name ) ); ?>
		</p>
	</header>
	...
	<div class="ai-news-archive-layout" style="display: flex; gap: 40px; align-items: flex-start; flex-wrap: wrap;">
		<main class="ai-news-main-content" style="flex: 1; min-width: 320px;">
			...
			$fallback_items = aikairali_get_fallback_glossary_for_category( $cat_slug, $cat_name );
			?>
			...
			<div class="ai-news-list-stream">
				...
				if ( $cat_query->have_posts() ) :
					while ( $cat_query->have_posts() ) : $cat_query->the_post();
						aikairali_render_glossary_list_item( get_the_ID() );
					endwhile;
					wp_reset_postdata();
				else :
					foreach ( $fallback_items as $fitem ) :
						aikairali_render_glossary_default_item( $fitem );
					endforeach;
				endif;
				?>
			</div>
		...
		<aside class="ai-news-sidebar" style="width: 340px; min-width: 280px; box-sizing: border-box;">
			...
			<div class="ai-news-recent-widget" style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 16px; padding: 22px;">
				<div style="display: flex; align-items: center; gap: 8px; margin-bottom: 20px; padding-bottom: 12px; border-bottom: 2px solid #0f172a;">
					...
					<h3 style="font-size: 13px; font-weight: 800; color: #0f172a; margin: 0; text-transform: uppercase; letter-spacing: 0.06em;">RECENT TERMS</h3>
				</div>
...
/**
 * Return customized fallback glossary items per category
 */
function aikairali_get_fallback_glossary_for_category( $slug, $cat_name ) {
	$default_items = [
		[
			'title' => sprintf( '%s സാങ്കേതിക ശബ്ദകോശവും മലയാളം നിർവ്വചനങ്ങളും', $cat_name ),
			'excerpt' => 'നിർമ്മിത ബുദ്ധി സാങ്കേതിക പദങ്ങൾ എളുപ്പത്തിൽ മനസിലാക്കാൻ സഹായിക്കുന്ന ശബ്ദകോശം...',
			'image' => 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s മെഷീൻ ലേണിംഗ് പദാവലി വിവരണങ്ങൾ', $cat_name ),
			'excerpt' => 'ഡാറ്റാ സയൻസിലും അൽഗോരിതങ്ങളിലും ഉപയോഗിക്കുന്ന പ്രധാന പദങ്ങളുടെ സമഗ്ര വിവർത്തനം...',
			'image' => 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s ജനറേറ്റീവ് എഐ സാങ്കേതിക സംജ്ഞകൾ', $cat_name ),
			'excerpt' => 'ട്രാൻസ്‌ഫോർമറുകൾ, ലാർജ് ലാംഗ്വേജ് മോഡലുകൾ എന്നിവയെക്കുറിച്ചുള്ള തെളിമയുള്ള വിശദീകരണം...',
			'image' => 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s പ്രോംപ്റ്റിംഗ് ടെർമിനോളജി ഗൈഡ്', $cat_name ),
			'excerpt' => 'ചാറ്റ്ജിപിടി സന്ദേശങ്ങളിലും ഉത്തരങ്ങളിലുമുള്ള സാങ്കേതിക വാചകങ്ങൾ വിവരിക്കുന്നത്...',
			'image' => 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600&auto=format&fit=crop',
		],
		[
			'title' => sprintf( '%s എഐ സുരക്ഷയും എത്തിക്സ് നിഘണ്ടുവും', $cat_name ),
			'excerpt' => 'ആർട്ടിഫിഷ്യൽ ഇന്റലിജൻസ് നിബന്ധനകളും നീതിയുക്തമായ ഉപയോഗ നിയമങ്ങളും...',
...
/**
 * Helper function to render a single DB glossary item in stream list view
 */
function aikairali_render_glossary_list_item( $post_id ) {
	...
	$terms    = get_the_terms( $post_id, 'glossary-category' );
	$cat_name = ( $terms && ! is_wp_error( $terms ) ) ? mb_strtoupper( $terms[0]->name, 'UTF-8' ) : 'GLOSSARY';

	aikairali_render_glossary_default_item( [
		'title'   => $title,
		'excerpt' => $excerpt,
		'cat'     => $cat_name,
		'date'    => $date,
...
/**
 * Helper function to render a glossary item array structure matching stream design
 */
function aikairali_render_glossary_default_item( $item ) {
	?>
